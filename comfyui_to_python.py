...
class LoadOrderDeterminer:
    ...
    def determine_load_order(self) -> List[Tuple[str, Dict, bool]]:
        ...
        for key in self.data:
            ...
            if class_type in ["easy showAnything"]:
                continue
        ...
        for idx, data, is_special_function in load_order:
            ...
            for i, input in enumerate(list(input_types.get("required", {})) + list(input_types.get("optional", {}))):
                if isinstance(inputs.get(input, []), (int, str, float, bool)):
                    ...
                    print(f"CLI arg {input} for type {class_type} " + str(len(arg_inputs)))
    ...
    def assemble_python_code(self, import_statements: set, special_functions_code: List[str], arg_inputs: List[Tuple[str, str]], code: List[str], queue_size: int, custom_nodes=False, include_prompt_data=True) -> str:
        ...
        for i, (input_name, arg_desc, default) in enumerate(arg_inputs):
            ...
            if default is not None:
                extra = f"default={repr(default)}, "
            argparse_code.append(f'parser.add_argument("--{input_name}", {extra}help="{arg_desc} (autogenerated)")\n')
        argparse_code.append(f'parser.add_argument("--queue_size", "-q", type=int, default={queue_size}, help="How many times the workflow will be executed (default: {queue_size})")\n')
        argparse_code.append('parser.add_argument("--comfyui_directory", "-c", default=None, help="Where to look for ComfyUI (default: current directory)")\n')
        ...
        argparse_code.append(f'parser.add_argument("--disable_metadata", action="store_true", help="Disables writing workflow metadata to the outputs")\n')
